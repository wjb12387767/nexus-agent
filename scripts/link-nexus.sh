#!/bin/sh
# Install the dev `nexus` wrapper into Bun's global bin directory.
#
...
# `packages/coding-agent/scripts/nexus`. See that wrapper's header comment for the
...
# through that error would expand `$(bun pm -g bin)/nexus` to `/nexus` and try to
# write under `/` — see https://github.com/can1357/oh-my-pi/issues/3701.
set -e

repo_root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd -P)
target=$repo_root/packages/coding-agent/scripts/nexus

if [ ! -x "$target" ]; then
	echo "link-nexus: target wrapper not found or not executable: $target" >&2
	exit 1
fi
...
mkdir -p "$global_bin"
ln -sfn "$target" "$global_bin/nexus"
echo "link-nexus: linked $global_bin/nexus -> $target"
