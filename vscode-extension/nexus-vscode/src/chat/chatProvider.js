...
function getLaunchConfig() {
  ...
  const command = cfg.get('launchCommand', 'nexus');
  const shimEnabled = cfg.get('useOpenAIShim', false);
  const permissionMode = cfg.get('permissionMode', 'acceptEdits');
  // 传输模式：stdio（默认，spawn `nexus --print --input-format=stream-json`）
  // 或 grpc（连接到运行中的 `nexus grpc --port <port>` server，M5）
  const transportMode = cfg.get('transportMode', 'stdio');
  const grpcEndpoint = cfg.get('grpcEndpoint', 'localhost:50051');
  const env = {};
  if (shimEnabled) env.CLAUDE_CODE_USE_OPENAI = '1';
  const folders = vscode.workspace.workspaceFolders;
  const cwd = folders && folders.length > 0 ? folders[0].uri.fsPath : undefined;
  return { command, cwd, env, permissionMode, transportMode, grpcEndpoint };
}
...
class ChatController {
  ...
  async startSession(opts = {}) {
    ...
    const { command, cwd, env, permissionMode, transportMode, grpcEndpoint } = getLaunchConfig();

    // 根据传输模式选择后端：stdio 走 ProcessManager，grpc 走 GrpcTransport
    if (transportMode === 'grpc') {
      this._process = new GrpcTransport({
        command,
