...
const { ChatController, NexusChatViewProvider, NexusChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const NEXUS_REPO_URL = 'https://github.com/nexus-agent/nexus-agent';
const NEXUS_SETUP_URL = 'https://github.com/nexus-agent/nexus-agent/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.nexus-profile.json';
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('nexus');
  const launchCommand = configured.get('launchCommand', 'nexus');
  const terminalName = configured.get('terminalName', 'Nexus');
...
async function launchNexus(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Nexus command not found: ${executable}. Install it with: npm install -g nexus-agent@latest`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(NEXUS_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(NEXUS_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch Nexus because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Nexus wordmark">Ne<span class="wordmark-accent">xus</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream Nexus project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and Nexus commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
class NexusControlCenterProvider {
  constructor() {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          ...
          break;
        case 'launchRoot':
          await launchNexus({ requireWorkspace: true });
          break;
        case 'openProfile':
...
function activate(context) {
  ...
  const chatViewProvider = new NexusChatViewProvider(chatController);
  const chatPanelManager = new NexusChatPanelManager(chatController);
  ...
  statusBarItem.tooltip = 'Open Nexus Chat';
  ...
  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) Nexus';
        statusBarItem.tooltip = 'Nexus is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) Nexus';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('nexus.start', async () => {
    await launchNexus();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'nexus.startInWorkspaceRoot',
    async () => {
      await launchNexus({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('nexus.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(NEXUS_REPO_URL));
  });
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'nexus.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('nexus.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.nexus');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('nexus.newChat', () => {
  ...
  const resumeSessionCommand = vscode.commands.registerCommand('nexus.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('nexus.abortChat', () => {
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('nexus')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  NexusControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  NexusChatViewProvider,
  NexusChatPanelManager,
};
