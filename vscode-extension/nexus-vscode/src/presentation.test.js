...
test('truncateMiddle keeps the profile filename visible', () => {
  ...
  assert.equal(
    truncateMiddle('/Users/example/projects/nexus/workspace/.nexus-profile.json', 30),
    '.../.nexus-profile.json',
  );
});
...
test('truncateMiddle keeps the filename visible for Windows-style paths', () => {
  ...
  assert.equal(
    truncateMiddle('C:\\Users\\example\\nexus\\workspace\\.nexus-profile.json', 30),
    '...\\.nexus-profile.json',
  );
});
...
function createStatus(overrides = {}) {
  return {
    ...
    terminalName: 'Nexus',
    shimEnabled: false,
    workspaceFolder: '/workspace/nexus',
    ...
    profileStatusHint: '/workspace/nexus/.nexus-profile.json',
    workspaceProfilePath: '/workspace/nexus/.nexus-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
      ...
      rows: [
        {
          ...
          summary: 'nexus',
          detail: '/workspace/nexus · active editor workspace',
        },
        {
          key: 'profileStatus',
          label: 'Workspace profile',
          summary: 'Found',
          detail: '/workspace/nexus/.nexus-profile.json',
          tone: 'neutral',
        },
      ],
    },
...
test('buildControlCenterViewModel keeps launch command only in summary cards', () => {
  ...
  assert.deepEqual(viewModel.summaryCards.find(card => card.key === 'launchCommand'), {
    ...
    value: 'nexus --project-aware',
    detail: 'Integrated terminal: Nexus',
  });
