...
Magic keywords are standalone words in a user prompt that add a hidden instruction for that turn. They are enabled by default and glow in the editor when `nexus` recognizes them.
...
nexus config set magicKeywords.enabled false

# Disable one keyword while leaving the others enabled
nexus config set magicKeywords.ultrathink false
nexus config set magicKeywords.orchestrate false
nexus config set magicKeywords.workflow false
```

All four settings default to `true`. Run `nexus config list` to inspect every available setting and its current value. See [Settings](./settings.md) for configuration scopes, precedence, and project-local overrides.
