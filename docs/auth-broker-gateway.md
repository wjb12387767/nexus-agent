...
- **`nexus auth-broker serve`** holds the canonical SQLite credential vault, performs OAuth refreshes, and exposes a small REST API (`/v1/snapshot`, `/v1/snapshot/stream`, `/v1/credential/:id/refresh`, `/v1/credential/:id/disable`, `/v1/credential`, `/v1/usage`, `/v1/healthz`).
- **`nexus auth-gateway serve`** is a forward-proxy. It accepts OpenAI Chat Completions, Anthropic Messages, OpenAI Responses, and pi-native stream requests, resolves the broker-backed credential, and dispatches through `pi-ai` provider logic. Clients (containerised nexus, llm-git, the macOS usage widget, …) never see the access token.
...
```
                ...
                │                                                            │
  developer ──▶ │  ┌──────────────────────────┐    ┌────────────────────┐    │
  laptop /      │  │  nexus auth-broker serve   │◀──▶│  SQLite agent.db    │    │
  CI / robomp   │  │  - holds refresh tokens  │    │  (canonical writer)│    │
                ...
                │            ▼                                               │
                │  ┌──────────────────────────┐                              │
                │  │  nexus auth-gateway serve  │  RemoteAuthCredentialStore   │
...
```
nexus auth-broker serve     [--bind=host:port]                    # boot the broker
nexus auth-broker token     [--regenerate] [--json]               # print or rotate the bearer token
nexus auth-broker login     [<provider>] [--via=user@host] [--dry-run]
nexus auth-broker logout    [<provider>]
nexus auth-broker list      [--json]
nexus auth-broker import    <file|dir> [--provider=<id>] [--include-disabled] [--dry-run] [--json]
nexus auth-broker migrate   --from-local [--include-oauth] [--include-env] [--dry-run] [--json]
nexus auth-broker status    [--json]
```
...
- `import <file|dir>` imports CLIProxyAPI-style JSON credentials into the local SQLite store. Maps `type` field → nexus provider (`claude → anthropic`, `codex → openai-codex`, `gemini → google-gemini-cli`, `antigravity → google-antigravity`, `gemini-cli → google-gemini-cli`).
...
```
nexus auth-gateway serve   [--bind=host:port] [--no-auth]
nexus auth-gateway token   [--regenerate] [--json]
nexus auth-gateway status  [--json]
nexus auth-gateway check   [--strict] [--json]
```
...
The model id is read from the top-level `model` field for foreign wire formats and from the pi-native request body for `/v1/pi/stream`. The gateway picks the first bundled `Model<Api>` matching that id, parses the inbound wire format into an nexus `Context`, resolves the provider credential from broker-backed `AuthStorage`, dispatches through `streamSimple()`, and re-encodes the result to the inbound format (SSE for streamed responses).
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `OMP_AUTH_BROKER_URL`   | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`). Selecting this puts the client in broker mode — local SQLite is bypassed. | Any time the nexus client should resolve credentials through a broker (and required by `nexus auth-gateway serve`).           |
...
| `OMP_AUTH_BROKER_SNAPSHOT_CACHE`  | Path override for the encrypted local snapshot cache. Default `~/.nexus/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). | Optional in broker mode. |
...
| Path                              | Owner                                                | Mode                          |
| --------------------------------- | ---------------------------------------------------- | ----------------------------- |
| `<config-dir>/auth-broker.token`  | `nexus auth-broker serve` (created at first start)     | `0600` in a `0700` parent dir |
| `<config-dir>/auth-gateway.token` | `nexus auth-gateway serve` (skipped under `--no-auth`) | `0600` in a `0700` parent dir |

`<config-dir>` resolves to `~/.nexus/` (respecting `PI_CONFIG_DIR`).
