...
The native provider is the recommended format for new projects. It reads from your user agent directory and from `.nexus/` directories inside a project, and it has the highest discovery priority, so its files win over every other convention at the same scope.

| File | Scope | Behavior |
|---|---|---|
| `~/.nexus/agent/AGENTS.md` | User | User-level context for every session unless the `native` provider is disabled. |
| `<ancestor>/.nexus/AGENTS.md` | Project | Project context. `nexus` walks upward from the current directory to the repository root and uses the **nearest** non-empty `.nexus/AGENTS.md`. Farther native project files are not also included. |
| `~/.nexus/agent/RULES.md` | User | User-level sticky rule content. Loaded as an always-apply rule, not as a context file. |
| `<ancestor>/.nexus/RULES.md` | Project | Project sticky rule content. Same nearest-ancestor walk-up as above. Loaded as an always-apply rule. |

Two details matter:

- **Walk-up to the repository root.** Discovery starts in the current working directory and climbs through each ancestor up to the repository root, stopping at the first ancestor that has a usable `.nexus/` directory. The *nearest* match wins; ancestors above it are not loaded as native context.
- **The `.nexus/` directory must be non-empty.** An empty `.nexus/` directory is skipped during the walk-up, so the search continues to the next ancestor. An empty `AGENTS.md` or `RULES.md` file contributes nothing.

`~/.nexus/agent` is the user base. If `PI_CODING_AGENT_DIR` is set, it relocates that base, so the user files become `$PI_CODING_AGENT_DIR/AGENTS.md` and `$PI_CODING_AGENT_DIR/RULES.md`.
...
- The native context file is `repo/packages/api/.nexus/AGENTS.md` (the nearest one). `repo/.nexus/AGENTS.md` is **not** also included.
- The project sticky rule is `repo/packages/api/.nexus/RULES.md` if present; otherwise the walk-up continues and `repo/.nexus/RULES.md` is used.
...
`nexus` also discovers the context and rule files of other agent tools so existing projects keep working without migration.

| Provider id | Convention path | Scope | Notes |
|---|---|---|---|
| `native` | `.nexus/AGENTS.md` | User + project | Recommended `nexus` format. User file at `~/.nexus/agent/AGENTS.md`; project file is the nearest non-empty `.nexus/AGENTS.md` walking up to the repo root. |
...
Providers marked "(no ancestor walk-up)" only look in the current working directory's config directory. If you need ancestor walk-up behavior, prefer the native `.nexus/AGENTS.md` format or a standalone `AGENTS.md` (the `agents-md` provider), or launch `nexus` from the directory that holds the config directory.
...
- **One user context file** is kept across all providers. Because `native` has the highest priority, `~/.nexus/agent/AGENTS.md` shadows every other user-level context file.
...
- If you add `repo/packages/api/.nexus/AGENTS.md`, `native` (priority 100) wins depth 0 outright, shadowing both lower-priority files.
...
- It is read **only** at the native locations — `~/.nexus/agent/RULES.md` and the nearest `<ancestor>/.nexus/RULES.md` from the cwd up to the repo root. A `RULES.md` anywhere else is not a context-file convention and is ignored.
...
Turn a provider off with the `disabledProviders` setting in `~/.nexus/agent/config.yml`, a project's `.nexus/config.yml`, or a `--config` overlay:
...
- Native project context must live at `.nexus/AGENTS.md`, and the `.nexus/` directory must be non-empty; an empty `.nexus/` is skipped and the walk-up continues to the next ancestor.
...
At one user scope or project depth, the higher-priority provider shadows the others (native > claude > agents/codex > gemini > opencode > github > agents-md). To force deterministic behavior, move your guidance into `.nexus/AGENTS.md` (native always wins) or disable the competing discovery provider.

### User context disappeared

Only one user-level context file survives, and `~/.nexus/agent/AGENTS.md` has the highest priority. If it exists, it shadows user-level `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, `~/.gemini/GEMINI.md`, `~/.config/opencode/AGENTS.md`, `~/.copilot/copilot-instructions.md`, and `~/.agent`/`~/.agents` files. Consolidate user guidance into the native file or remove the native one if you prefer another tool's file.

### A `RULES.md` file is ignored

Only the native `RULES.md` locations are sticky: `~/.nexus/agent/RULES.md` and the nearest `<ancestor>/.nexus/RULES.md` from cwd to the repo root. A `RULES.md` in any other directory is not a recognized convention and will not be loaded.
