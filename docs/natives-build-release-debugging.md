...
3. Runtime candidate order includes:
   ...
   - versioned cache dir,
   - legacy compiled-binary dir (`%LOCALAPPDATA%/nexus` on Windows, `~/.local/bin` elsewhere),
...
- Root: `/data/cache/pi-natives` (provisioned by `entrypoint.sh` alongside the cargo caches, owned `root:nexus`, mode `02770` setgid so cached files inherit `gid=nexus` and stay readable by every slot user).
...
| `ROBOMP_NATIVES_CACHE_ROOT`                 | `/data/cache/pi-natives` | Cache root directory. Must be `root:nexus 02770` for cross-slot reads.                                |
