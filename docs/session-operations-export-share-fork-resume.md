...
`loadCustomShare()` checks `~/.nexus/agent` for first existing candidate:
...
4. Upload target is chosen by `share.store`:
   - **Share server** (default, `store: "blob"`) — `POST <share.serverUrl>`
     (default `https://collab.nexus.agent/s`) with the raw blob, capped at 1 MB.
...
Use `--prompt-cache-key <key>` to pin the provider prompt-cache identity explicitly and independently from both the nexus session id and `--provider-session-id`. `--provider-session-id` continues to control provider session/routing headers and sticky credential selection; `--prompt-cache-key` controls the OpenAI Responses `prompt_cache_key` payload where supported.
