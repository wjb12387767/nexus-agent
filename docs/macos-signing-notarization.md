...
The compiled macOS `nexus` binaries shipped on GitHub Releases are signed with a
...
- `curl https://nexus.agent/install | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/nexus-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/nexus-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/nexus-signing             # upload all five
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/nexus-darwin-arm64
```
