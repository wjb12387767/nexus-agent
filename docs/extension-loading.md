...
- Project directory: `<cwd>/.nexus/extensions`
- User directory: `~/.nexus/agent/extensions`
- Native legacy/settings JSON entries: `<cwd>/.nexus/settings.json#extensions` and `~/.nexus/agent/settings.json#extensions`

The project root is the native provider's `.nexus` directory (`SOURCE_PATHS.native.projectDir`), cwd-only; it does not walk ancestors. The user root is the active profile's agent directory via `getAgentDir()`, so under `nexus --profile <name>` it becomes `~/.nexus/profiles/<name>/agent/extensions` (and it honors `PI_CODING_AGENT_DIR`). See [Profiles](./config-usage.md#profiles).

Notes:

- Native auto-discovery is currently `.nexus` based.
...
Plugin extension entries come from package `nexus.extensions` / `pi.extensions` manifests, including enabled feature entries.
...
- User: `~/.nexus/agent/config.yml` (or custom agent dir via `PI_CODING_AGENT_DIR`)
- Project/native settings capability: `<cwd>/.nexus/config.yml` and `<cwd>/.nexus/settings.json`

Native extension-module discovery also reads legacy JSON extension lists from:

- `~/.nexus/agent/settings.json`
- `<cwd>/.nexus/settings.json`
...
```json
{
  "extensions": ["./.nexus/extensions/my-extra"]
}
```
...
1. `package.json` in that directory with `nexus.extensions` (or legacy `pi.extensions`) -> use declared entries
2. `index.ts`
3. `index.js`
4. Otherwise scan one level for extension entries:
   - direct `*.ts` / `*.js`
   - subdir `index.ts` / `index.js`
   - subdir `package.json` with `nexus.extensions` / `pi.extensions`
