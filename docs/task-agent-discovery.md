...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from OMP-native roots and Claude plugin roots before appending bundled definitions. Cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped — their frontmatter schema is not the nexus task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".nexus"` filters both dir lists).

### Discovery inputs

1. Nearest project `.nexus` agents dir from `findAllNearestProjectConfigDirs("agents", cwd)` (filtered to `.nexus`; first hit only)
2. User `.nexus` agents dir from `getConfigDirs("agents", { project: false })` (filtered to `.nexus`; first hit only)
...
1. project `.nexus/agents`
2. user `~/.nexus/agent/agents`
...
- Project `.nexus` overrides user `.nexus`.
