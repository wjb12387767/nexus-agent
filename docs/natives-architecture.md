...
2. Check legacy compiled-binary location:
   - Windows: `%LOCALAPPDATA%/nexus` (fallback `%USERPROFILE%/AppData/Local/nexus`)
   - non-Windows: `~/.local/bin`
3. Fall back to packaged `native/` and executable directory candidates.

`getNativesDir()` uses `$XDG_DATA_HOME/nexus/natives` when `$XDG_DATA_HOME/nexus` exists; otherwise it uses `~/.nexus/natives`.
