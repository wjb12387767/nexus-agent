...
A **plugin** is a directory containing Claude/nexus plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Extension modules (`package.json` `nexus.extensions` entry points) are not loaded from marketplace installs — they only load for npm-installed or `nexus plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.nexus/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.nexus/plugins/installed_plugins.json`
...
```
nexus plugin marketplace add <source>
nexus plugin marketplace remove <name>
nexus plugin marketplace update [name]
nexus plugin marketplace list
nexus plugin discover [marketplace]
nexus plugin install [--force] [--scope user|project] name@marketplace
nexus plugin uninstall [--scope user|project] name@marketplace
nexus plugin upgrade [--scope user|project] [name@marketplace]
nexus plugin enable [--scope user|project] name@marketplace
nexus plugin disable [--scope user|project] name@marketplace
```
...
A marketplace catalog lives at `.omp-plugin/marketplace.json` in the repository root. When nexus is the only intended consumer, prefer this path. To remain Claude Code-compatible (nexus loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — nexus uses it as a fallback when `.omp-plugin/marketplace.json` is absent. A repository may ship both: nexus reads the `.omp-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
