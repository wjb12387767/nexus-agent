...
## 第二十四章 桌面端架构（WorkAny 集成）

Nexus Agent 的桌面端形态名为 **WorkAny**，是一个独立的 Tauri + React 应用，通过子进程桥接方式复用 nexus-agent 的 CLI 内核。本章深入解析 WorkAny 的架构设计、适配器实现、设置同步、工作空间选择、消息撤回、热力图可视化与打包流程。

### 24.1 WorkAny 架构概述

WorkAny 采用 **Tauri 壳 + React 前端 + Hono sidecar 后端** 的三层架构：

| 层 | 技术栈 | 职责 | 代码位置 |
|---|---|---|---|
| **Tauri 壳** | Rust + tauri-runtime | 窗口管理、系统对话框、文件系统权限、原生菜单、应用打包 | `src-tauri/` |
| **前端** | React + React Router + Tailwind | UI 渲染、用户交互、状态管理、i18n、设置面板 | `src/` |
| **后端 sidecar** | Hono (Bun runtime) | Agent 调度、Provider 管理、MCP 加载、沙箱池、Skills 加载、文件/历史 API | `src-api/` |

**为什么是 Hono sidecar 而非 Tauri command？**

nexus-agent 的 Agent 内核依赖 Bun 运行时与 monorepo workspace，无法直接以 Node.js 模块形式导入 Tauri 的 Rust 后端。因此 WorkAny 把后端独立编译为 `bun build --compile` 产出的单二进制 sidecar（`workany-api-<target>`），由 Tauri 在启动时拉起作为本地 HTTP 服务，前端通过 fetch 调用。这种"壳 + sidecar"模式解耦了运行时依赖：Tauri 负责原生能力，Hono 负责 Agent 编排，nexus CLI 负责实际编码。

**目录结构要点**：

- `src/app/router.tsx` — 路由入口，`main.tsx` 通过 `RouterProvider` 挂载，不依赖旧版 `App.tsx`。
- `src/components/settings/tabs/` — 11 个设置 Tab，其中 `NexusSettings.tsx` 是 nexus-agent 专属设置面板。
- `src-api/src/extensions/agent/nexus/` — NexusAgent 适配器实现。
- `src-api/src/extensions/agent/codeany/` — 另一个 Agent 后端（对比项）。

### 24.2 NexusAgent 适配器设计（子进程桥接方案）

WorkAny 的 Agent 抽象层定义了 `BaseAgent` 接口与 `AgentPlugin` 插件机制。NexusAgent 是该接口的官方实现，位于 `src-api/src/extensions/agent/nexus/index.ts`。

**核心设计：子进程 + JSON 事件流**

由于 nexus-agent 是 Bun/TS 项目，无法被 Node.js 直接 import，适配器采用**子进程桥接**方案：

1. **CLI 探测**（`findNexusCli`）：按优先级查找 nexus 二进制——
   - 配置中显式指定的 `config.nexusPath`；
   - `PATH` 与常见安装路径（Unix: `/usr/local/bin`、`/opt/homebrew/bin`、`~/.local/bin`；Windows: `%LOCALAPPDATA%`、`.exe`/`.cmd`/`.bat` 扩展名）；
   - `NEXUS_CLI_FALLBACK_NAMES` 候选名列表。

2. **子进程拉起**（`runNexusSubprocess`）：用 `child_process.spawn` 启动 `nexus --print --json --cwd <session> <prompt>`，以 `stdio: ['ignore', 'pipe', 'pipe']` 捕获 stdout/stderr。

3. **事件流解析**（`parseNexusEvent`）：逐行读取 stdout，按 `type` 字段分发为统一的 `AgentMessage`：

   | nexus 事件 type | 转换为 AgentMessage.type | 说明 |
   |---|---|---|
   | `agent_start` | `session` | 会话开始，携带 sessionId |
   | `message_update` | `text`（isDelta=true） | 文本增量，取 `delta.text_delta` |
   | `tool_execution_start` | `tool_use` | 工具调用开始 |
   | `tool_execution_end` | `tool_result` | 工具调用结束 |
   | `agent_end` | `result` | 携带 cost 与 duration |
   | `error` | `error` | 错误事件 |
   | 非 JSON 行 | `text`（isDelta=true） | 降级为纯文本增量 |

   解析采用**防御式策略**：字段同时兼容 snake_case 与 camelCase（如 `session_id` / `sessionId`），未知事件类型若含 `text` 字段则降级为文本增量，否则跳过。

4. **三种执行模式**：`run`（直接执行）、`plan`（仅规划，加 `--plan-only`）、`execute`（执行已存储的 plan）。Plan 模式会收集完整文本响应后解析为结构化步骤。

5. **会话工作目录隔离**（`getSessionWorkDir`）：每个会话在 `<workDir>/sessions/<slug>` 下独立工作，slug 由 prompt 生成（去除中文、截断 50 字符、追加 taskId 后 6 位），确保多任务互不干扰。

6. **生命周期管理**：`childProcesses` Map 跟踪每个会话的子进程，`stop()` 发送 SIGTERM，`shutdown()` 批量清理。`AbortController` 支持流式中断。

**错误分类**（`classifyError`）：将 stderr 文本匹配为三类——API Key 错误（`__API_KEY_ERROR__`）、自定义 API 错误（`__CUSTOM_API_ERROR__`）、内部错误（`__INTERNAL_ERROR__`），便于前端精准提示。

### 24.3 设置系统集成（28 项设置 → CLI 参数/环境变量）

WorkAny 设置面板的 **NexusSettings Tab**（`src/components/settings/tabs/NexusSettings.tsx`）把 nexus-agent 的能力项暴露为 GUI 开关，组织为 **7 个可折叠分区**：

| 分区 | 设置项 | 默认值 |
|---|---|---|
| **核心 Agent** | model · maxTurns · permissionMode · thinkingLevel | sonnet-4 · 200 · bypass · adaptive |
| **沙箱** | sandboxEnabled · sandboxFallbackBehavior · bashAstSafety · fileSafetyEnabled · doomLoopDetection | true · warn · true · true · true |
| **检查点** | checkpointEnabled · autoCheckpointInterval · maxCheckpoints | true · 300 · 50 |
| **上下文与记忆** | compactionEnabled · compactionThreshold · mnemopiEnabled | true · 75 · false |
| **自我改进** | autoLearnEnabled · backgroundReviewEnabled · curatorEnabled · learningGraphEnabled · autoLearnCapture · reflectOnErrors | true · false · false · true · true · true |
| **WSL 桥接** | wslAutoDetect · wslPreferredDistro · wslSuppressHint | true · (空) · false |
| **高级** | promptCaching · contextBreakdown · thinkScrubber · telemetry · debugMode | true · false · true · false · false |

**设置到子进程的映射**：适配器的 `buildNexusArgs()` 与 `buildEnv()` 两套通道把上述设置注入 nexus CLI——

- **CLI 参数通道**（`--print --json --cwd --model --max-turns --permission-mode --thinking --sandbox --checkpoint --api-base`）：传给 spawn 的 args 数组。
- **环境变量通道**（`NEXUS_FILE_SAFETY` / `NEXUS_DOOM_LOOP` / `NEXUS_BASH_AST` / `NEXUS_COMPACTION_ENABLED` / `NEXUS_AUTO_LEARN` / `NEXUS_BACKGROUND_REVIEW` / `NEXUS_CURATOR` / `NEXUS_MNEMOPI` / `NEXUS_LEARNING_GRAPH` / `NEXUS_PROMPT_CACHING` / `NEXUS_THINK_SCRUBBER` 等）：布尔项编码为 `"true"`/`"false"`，数值项编码为字符串，nexus CLI 读取时未知变量安全忽略。

这种双通道设计的原因：部分能力项（如沙箱、检查点）有专用 CLI flag，而大量自我改进/优化项更适合走环境变量，避免 CLI 参数爆炸。`apiKey` 根据 `apiType` 自动映射为 `ANTHROPIC_API_KEY` 或 `OPENAI_API_KEY`。

### 24.4 工作空间选择（Tauri dialog + Web showDirectoryPicker）

WorkAny 支持用户选择工作目录作为 Agent 的工作空间：

- **桌面端**：通过 Tauri 的 `@tauri-apps/plugin-dialog` 调用原生目录选择器（`open` + `directory: true`），返回的路径经 `expandPath` 展开 `~`、在 Windows 下把 `/` 转为 `\`，最终作为 `--cwd` 传给 nexus 子进程。
- **Web 端**：降级为浏览器的 `window.showDirectoryPicker()`（File System Access API），通过权限句柄访问目录。

选中路径会持久化到设置数据库（`workDir` 字段），并在每次会话创建时由 `getSessionWorkDir` 派生出隔离的 `sessions/<slug>` 子目录，保证多任务文件互不覆盖。

### 24.5 消息撤回/回滚机制

桌面端与 Web 端支持**消息撤回**，终端不支持（因 TUI 的 append-only 语义，见第十五章）。撤回机制建立在 nexus-agent 的 checkpoint 之上：

1. **自动检查点**：当 `autoCheckpointInterval` 触发或工具执行前，nexus CLI 创建文件级快照（Linux/macOS 走 reflink CoW O(1)，Windows 走全量拷贝 O(N)）。
2. **撤回触发**：用户在消息上选择"撤回"时，前端调用历史 API 标记该消息及其后的工具结果为已撤回。
3. **回滚执行**：适配器定位撤回点对应的 checkpoint，调用 nexus 的 rewind 能力恢复文件状态到该快照。
4. **会话续接**：撤回后的会话上下文被截断到撤回点，后续 Agent 调用以截断后的历史为上下文继续。

这一机制让用户能"后悔"——撤销 Agent 的一轮错误操作（如误删文件、错误编辑），而不仅是删除消息显示。

### 24.6 热力图可视化

桌面端与 Web 端提供**热力图分析**（AnalyticsSettings Tab），将 Agent 的工作强度可视化：

- 以日历热力图展示每日任务数、工具调用次数、Token 消耗。
- 数据来源于 `src-api` 的 `history.ts` 与 `stats` 服务，聚合自本地 SQLite（`src/shared/db/database.ts`）。
- 终端不提供此能力（TUI 无图形渲染），用户需切换到桌面/Web 端查看分析。

### 24.7 打包流程（build-nexus.sh + CI）

`scripts/build-nexus.sh` 是一键构建脚本，把 nexus CLI 与 workany-api 编译为 sidecar 并打包为平台安装包：

**构建管线**：

1. **依赖检查**：要求 `pnpm` + `cargo`/`rustup` + `bun`（nexus-agent 与 sidecar 均需 Bun）。
2. **workany-api sidecar**：`bun build src/index.ts --compile --target=<bun_target> --outfile dist/workany-api-<rust_target>`，产出单二进制后端。
3. **nexus CLI 捆绑**（`--with-nexus` 开关）：到 nexus-agent 仓库根，`bun --cwd=packages/coding-agent run build` 编译 nexus CLI，按 `CROSS_TARGET` 交叉编译，产物拷贝为 `src-api/dist/nexus-<rust_target>`。
4. **前端构建**：`pnpm build`。
5. **Tauri 打包**：`pnpm tauri build --target <rust_target>`，产出 `.dmg`/`.deb`/`.rpm`/`.msi`/`.exe`/`.AppImage`。

**目标三元组映射**：

| 平台别名 | Rust target | Bun compile target | nexus cross target |
|---|---|---|---|
| `current` | 自动探测 | 自动探测 | (native) |
| `linux` | `x86_64-unknown-linux-gnu` | `bun-linux-x64` | `linux-x64` |
| `windows` | `x86_64-pc-windows-msvc` | `bun-windows-x64` | `win32-x64` |
| `mac-arm` | `aarch64-apple-darwin` | `bun-darwin-arm64` | `darwin-arm64` |
| `mac-intel` | `x86_64-apple-darwin` | `bun-darwin-x64` | `darwin-x64` |
| `all` | 依次构建上述全部 | — | — |

**CI**：`.github/workflows/build-nexus.yml` 在 tag 推送时触发跨平台矩阵构建，产物上传到 GitHub Releases。Tauri sidecar 命名约定 `nexus-<rust_target>[.exe]`，运行时由 `NEXUS_CLI_FALLBACK_NAMES` 发现。

**为什么不能用 pkg 打包 nexus？** nexus-agent 依赖 Bun 运行时与 NAPI 原生模块（`.node`），Node.js 的 pkg 无法打包这些依赖，故采用 `bun build --compile` 编译为自带运行时的单二进制。

### 24.8 三端功能对齐表

| 功能 | 终端 (nexus CLI) | 桌面端 (WorkAny) | Web 端 |
|---|---|---|---|
| Agent 对话 | ✅ | ✅ | ✅ |
| 工具调用 | ✅ | ✅ | ✅ |
| 沙箱隔离 | ✅ | ✅ | ✅ |
...
| Skills | ✅ | ✅ | ✅ |
| 文件操作 | ✅ | ✅ | ✅ |
| 工作空间选择 | ✅ (CLI `--cwd`) | ✅ (Tauri dialog) | ✅ (showDirectoryPicker) |
| 设置面板 | ✅ (`nexus config`) | ✅ (GUI) | ✅ (GUI) |
| 热力图分析 | ❌ | ✅ | ✅ |
| 消息撤回 | ❌ (append-only) | ✅ | ✅ |
| GUI 动画 | ❌ | ✅ | ✅ |

三端共享同一个 nexus CLI 内核与 Agent loop，桌面/Web 端通过子进程桥接复用终端的全部能力，差异仅体现在交互层（GUI vs TUI）与可视化层（热力图、撤回）。

---

## 第二十五章 全平台安装指南

本章给出 nexus-agent 在 macOS、Linux、Windows 三平台的终端与桌面端安装方法。

### 25.1 macOS 安装

**方式一：Homebrew Cask（桌面端）**

```sh
brew install --cask workany
```

Cask 定义见仓库 `Casks/workany.rb`，安装后可在"应用程序"中找到 WorkAny。

**方式二：DMG 安装包（桌面端）**

从 GitHub Releases 下载 `WorkAny_<version>_aarch64.dmg`（Apple Silicon）或 `WorkAny_<version>_x64.dmg`（Intel），双击挂载后拖入"应用程序"。首次启动若提示"无法验证开发者"，在"系统设置 → 隐私与安全性"中点击"仍要打开"。

**方式三：curl 安装脚本（终端）**

```sh
curl -fsSL https://raw.githubusercontent.com/wjb12387767/nexus-agent/main/install.sh | bash
```

脚本会克隆仓库、安装 Bun + Rust（若缺失）、运行 `bun setup` 编译原生模块，并在 PATH 上创建 `nexus` 命令。首次编译 Rust NAPI 模块约需 10–20 分钟。
...
**方式一：deb / rpm / AppImage（桌面端）**

从 GitHub Releases 下载对应格式：

```sh
# Debian / Ubuntu
sudo dpkg -i WorkAny_<version>_amd64.deb

# Fedora / RHEL / openSUSE
sudo rpm -i WorkAny_<version>_x64.rpm

# AppImage（免安装，直接运行）
chmod +x WorkAny_<version>_x64.AppImage
./WorkAny_<version>_x64.AppImage
```
...
- 沙箱的 Landlock 后端需要 Linux 内核 ≥ 5.13（Landlock v1）。
- 检查点的 reflink CoW 需要 btrfs 或支持 reflink 的文件系统；其他文件系统降级为全量拷贝。
- Docker 镜像为 `linux/amd64`，arm64 需从源码构建。

### 25.3 Windows 安装

**方式一：MSI 安装包（桌面端）**

从 GitHub Releases 下载 `WorkAny_<version>_x64-setup.exe` 或 `WorkAny_<version>_x64_en-US.msi`，双击安装。

**方式二：PowerShell 安装脚本（终端）**

```powershell
irm https://raw.githubusercontent.com/wjb12387767/nexus-agent/main/install.ps1 | iex
```

**方式三：一键启动器（终端）**

仓库根目录的 `start.ps1` 会自动检测并安装 Bun / Rust、刷新 PATH、首次编译原生模块、启动开发服务器：

```powershell
.\start.ps1              # 自动安装工具链 + 构建 + 启动
.\start.ps1 -CheckOnly   # 仅检查 Bun / Rust
.\start.ps1 -SkipBuild   # 跳过原生模块编译
```

`start.bat` 是薄包装，优先调用 PowerShell 7（`pwsh`），回退到 Windows PowerShell 5.1。

**Windows 平台限制**（详见第二十三章）：

- 沙箱走 ISO FS（Projected FS）降级，无内核级 deny；建议用 WSL2 桥接（`nexus wsl launch`）获取完整 Landlock 沙箱。
- 检查点走全量拷贝（O(N)），无 reflink CoW。
- Bash 工具需 Git for Windows（Git Bash）；PowerShell 不支持作为 shell 后端。
- Docker 模式需 Docker Desktop + WSL2 后端。

### 25.4 桌面端：GitHub Releases 下载

桌面端（WorkAny）的安装包统一发布在 GitHub Releases：

- **macOS**：`.dmg`（Apple Silicon 与 Intel 分别提供）
- **Windows**：`.msi` 与 `.exe` 安装包
- **Linux**：`.deb` / `.rpm` / `.AppImage`

安装后打开应用，在设置面板配置 LLM API Key（Anthropic / OpenAI / DeepSeek / Qwen 等），选择工作空间即可开始使用。桌面端会自动探测系统已安装的 `nexus` CLI；若未安装，可从设置中指定 CLI 路径或使用内置 sidecar。
