...
- `~/.nexus/agent/sessions/<dir-encoded>/*.jsonl` (home-relative `-<rel>` names, `-tmp-<rel>` for temp paths, legacy `--<abs>--` otherwise)
...
1. Read terminal-scoped breadcrumb (`~/.nexus/agent/terminal-sessions/<terminal-id>`)
