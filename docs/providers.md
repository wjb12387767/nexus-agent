...
Providers are the model backends `nexus` can route requests to: Anthropic, OpenAI, Google Gemini, Groq, OpenRouter, Mistral, xAI, local engines like Ollama, hosted gateways, custom `models.yml` providers, and providers registered by extensions.
...
## How `nexus` decides a provider is available
...
2. Custom provider and model entries from `~/.nexus/agent/models.yml`.
...
When a provider needs an API key, `nexus` resolves it in this order (first match wins):
...
Stored credentials live in the auth store at `~/.nexus/agent/agent.db` for local auth, or in the configured auth-broker snapshot when running in broker mode. (`PI_CODING_AGENT_DIR` relocates the `~/.nexus/agent` base, and the auth store moves with it.)
...
For headless or remote setups backed by a shared auth broker, the CLI exposes `nexus auth-broker login <provider>` / `nexus auth-broker logout` (and `status`, `list`, `import`, `migrate`). See [Secrets and credentials](./secrets.md) for the broker model.

When a model has no credentials, `nexus` tells you to run `/login` or set the provider's environment variable.
...
`nexus` eagerly loads `.env` files into the process environment before any provider lookup. It reads four files and, for each variable, the **first** source that defines it wins. Effective precedence, high to low:

1. The process environment inherited by `nexus` (already-set variables always win).
2. `<cwd>/.env`
3. `~/.nexus/agent/.env`
4. `~/.nexus/.env`
5. `~/.env`

A variable already present in the process environment is never overwritten by a `.env` file. Among the files, a value set in `<cwd>/.env` wins over `~/.nexus/agent/.env`, which wins over `~/.nexus/.env`, which wins over `~/.env`. So a shell-exported `OPENAI_API_KEY` beats every `.env` file, and a project's `<cwd>/.env` beats your home `~/.env`.
...
```yaml
# ~/.nexus/agent/config.yml or <project>/.nexus/config.yml
disabledProviders:
  - anthropic
  - openai
  - google
...
Project settings live in `<project>/.nexus/config.yml`. Use them when one repository must allow or hide a different provider set than your global default:

```yaml
# <project>/.nexus/config.yml
disabledProviders:
  - openai
  - openrouter
```
...
Custom providers live in `~/.nexus/agent/models.yml` under `providers:`. A provider ID defined there participates in the same selection, credential resolution, and `disabledProviders` rules as built-in providers.
...
**A provider still appears even though I disabled it.** `disabledProviders` arrays are replaced, not merged: a project `<project>/.nexus/config.yml` array fully overrides the global one. Verify the *effective* list for the directory you are in (path-scoped entries only apply at or under their configured path), and confirm the ID is spelled exactly. Use `nexus config get disabledProviders` to inspect the merged value (see [Settings](./settings.md)).
