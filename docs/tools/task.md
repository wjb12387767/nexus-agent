...
- Isolation merge strategy: patch mode (capture/apply root patches) or branch mode (commit to `nexus/task/<id>`, cherry-pick into parent).
...
- Agent discovery precedence is first-wins by exact name: project `.nexus` agents dir before the user `.nexus` dir (task agents only load from `.nexus` roots; `.claude`/`.codex`/`.gemini` agent dirs are skipped), Claude plugin agent dirs after config dirs, bundled agents last. Create-time discovery is memoized per cwd for the prompt description; execution-time discovery stays fresh.
