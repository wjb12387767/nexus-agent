...
```text
~/.nexus/agent/sessions/<dir-encoded>/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.nexus/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.nexus/agent/terminal-sessions/<terminal-id>
```
...
- `listAllSessions(storage)` (a.k.a. `SessionManager.listAll()`) -> sessions across all project scopes under `~/.nexus/agent/sessions`
