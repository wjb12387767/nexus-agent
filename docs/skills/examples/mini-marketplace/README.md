...
```
nexus plugin marketplace add ./docs/skills/examples/mini-marketplace
nexus plugin install my-plugin@example-marketplace
```
...
mini-marketplace/
  ...
  README.md
  my-plugin/
    package.json          ← nexus.extensions manifest
    index.ts              ← extension entry point
```

Published and local marketplaces use the same catalog location. nexus loads `.omp-plugin/marketplace.json` first and falls back to `.claude-plugin/marketplace.json` (the Claude Code-compatible path this example ships) inside the marketplace root. Point `/marketplace add` at this folder to load the example.
