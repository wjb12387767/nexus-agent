...
```
cp -r . ~/.nexus/agent/extensions/hello-extension
```

Restart `nexus`. You will see the startup notification immediately.
...
```
nexus --extension ./hello-extension
```

## Usage

After loading, type `/hello` or `/hello Ada` in the nexus prompt. The command sends a visible greeting custom message into the conversation and shows a "Message sent!" notification.
...
- `package.json` with `nexus.extensions` manifest field
