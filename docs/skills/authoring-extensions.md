...
description: Use when creating a new nexus extension. Covers ExtensionAPI, factory signature, tool/command/event registration, and local-dev testing.
---
...
That is a working extension. Drop it into `~/.nexus/agent/extensions/hello.ts` and restart nexus to see the notification.
...
nexus loads extension modules from these sources:

1. Native `.nexus` locations discovered through the capability system:
   - `<cwd>/.nexus/extensions/`
   - `~/.nexus/agent/extensions/`
   - legacy extension paths listed in `.nexus/settings.json#extensions` or `~/.nexus/agent/settings.json#extensions`
2. Installed plugins under `~/.nexus/plugins/node_modules` (`nexus plugin install` npm/git specs, or `nexus plugin link`) via their `nexus.extensions`/`pi.extensions` manifests. Marketplace cache installs do not feed extension modules — they surface skills/commands/hooks/tools/MCP only.
3. Explicit configured paths passed by the CLI (`nexus --extension ./my-ext.ts`, also `-e`; `--hook` is treated as an alias) and by the `extensions:` setting in config.

The runtime de-duplicates by resolved absolute path — first seen wins.

When a path points to a directory, nexus resolves the entry point in this order:

1. `package.json` with `nexus.extensions` (or legacy `pi.extensions`) field
2. `index.ts`
3. `index.js`

When scanning an `extensions/` directory, nexus also loads direct `*.ts`/`*.js` files and one-level subdirectories that have `index.ts`, `index.js`, or a manifest.
...
To package an extension as an installable plugin, add an `nexus` field to `package.json`:
...
nexus writes structured logs to a rotating file under `~/.nexus/logs/` (debug level is always on; nothing is written to the console, which would corrupt the TUI). Tail today's log to see extension load diagnostics:

```
tail -f ~/.nexus/logs/nexus.$(date +%F).log
```
