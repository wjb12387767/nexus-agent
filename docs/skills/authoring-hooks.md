...
description: Use when creating a new nexus hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---
...
export default function myHook(nexus: HookAPI): void {
...
```ts
nexus.on("tool_result", async (event, ctx) => {
...
```ts
nexus.on("context", async (event, ctx) => {
...
export default function rmRfBlocker(nexus: HookAPI): void {
  nexus.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(nexus: HookAPI): void {
  nexus.on("tool_result", async (event) => {
...
export default function contextFilter(nexus: HookAPI): void {
  nexus.on("context", async (event) => {
