...
description: Use when creating a new nexus marketplace. Covers marketplace.json schema, source types, install commands, and publishing.
---
...
The catalog file lives at either `.omp-plugin/marketplace.json` or `.claude-plugin/marketplace.json` in the repository root. nexus prefers the `.omp-plugin/` path and falls back to the Claude path; a repository may publish both to expose tool-specific catalogs from a single source tree.
...
> Note: extension modules declared via `package.json` `nexus.extensions` are **not** loaded from marketplace installs — that mechanism only applies to npm-installed or `nexus plugin link`ed plugins. Ship marketplace plugin behavior through the conventional directories above.
...
```
nexus plugin marketplace add owner/repo
nexus plugin install name@marketplace-name
```

Scope behavior:

- **user** (default) — installed in `~/.nexus/plugins/installed_plugins.json`, available in all projects
- **project** — installed in `<project>/.nexus/plugins/installed_plugins.json`, available only in that project
