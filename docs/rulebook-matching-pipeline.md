...
Loads `.nexus` rules from:

- project: `<cwd>/.nexus/rules/*.{md,mdc}` when the cwd `.nexus` directory exists
- user: `~/.nexus/agent/rules/*.{md,mdc}`
- sticky user rule: `~/.nexus/agent/RULES.md`
- sticky project rule: nearest ancestor `.nexus/RULES.md` while walking from cwd toward the repository root
...
- `native` appends project `.nexus/rules`, user `~/.nexus/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
