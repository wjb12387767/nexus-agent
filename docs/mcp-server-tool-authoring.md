...
```text
Config sources (.nexus/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.nexus/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.nexus/mcp.json` (project) or `~/.nexus/agent/mcp.json` (user) for explicit control.
...
OMP-native MCP config (`.nexus/mcp.json`, `~/.nexus/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
