...
- `transport`: `pi-native` only. When set, every model under that provider is sent to an `nexus auth-gateway` compatible `baseUrl` via `POST /v1/pi/stream`; `apiKey` is the gateway bearer.
...
- `nexus models` prints provider-grouped tables of every concrete model, and `nexus models canonical` prints the coalesced canonical view
...
`disableStrictTools` is a provider-level flag that applies to all models in the provider. It disables the Anthropic `strict` marker only for tools that nexus would otherwise mark strict; it does not change runtime tool argument validation. nexus can automatically retry without strict tools after Anthropic reports a strict-grammar-too-large error before the first streamed token, but proxies that reject the `strict` field for other reasons should set this flag explicitly.
...
The built-in vLLM provider can be pointed at a non-default endpoint without declaring a custom discovery type. nexus uses vLLM's `/v1/models` metadata and preserves vLLM's `max_model_len` field as the discovered context window.
...
Most model configuration now flows through `models.yml` / `models.yaml` via `ModelRegistry`. Explicit `.json` / `.jsonc` paths remain supported only when passed programmatically to `ModelRegistry`; the default user config prefers `~/.nexus/agent/models.yml`, then falls back to `~/.nexus/agent/models.yaml`.
