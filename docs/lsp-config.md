# LSP configuration in nexus

This guide explains how to configure language servers for the nexus coding agent.
...
When no LSP config file is present, nexus auto-detects servers by intersecting two conditions:
...
nexus merges LSP config from multiple files, lowest to highest priority:

| Priority    | Location                                                                                                                    |
| ----------- | --------------------------------------------------------------------------------------------------------------------------- |
| 5 (lowest)  | `~/lsp.json`, `~/.lsp.json`, `~/lsp.yaml`, `~/.lsp.yaml`, `~/lsp.yml`, `~/.lsp.yml`                                         |
| 4           | Plugin LSP configs (marketplace / `--plugin-dir` roots)                                                                     |
| 3           | User config dirs: `~/.nexus/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.nexus/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.nexus/agent/lsp.json`
- Project-specific overrides → `<project>/.nexus/lsp.json`

> **Note:** Auto-detection is skipped only when at least one config file contributes server overrides. A config file that only sets `idleTimeoutMs` still lets nexus auto-detect built-in servers. When server overrides exist, nexus merges them with defaults and then loads servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
The `capabilities` object enables optional server-specific features that nexus supports on a per-server basis:
...
Place the override in `<project>/.nexus/lsp.json`:
...
The user-level config in `~/.nexus/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
