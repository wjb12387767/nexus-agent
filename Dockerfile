...
#                     + /usr/local/bin/nexus shim
#   pi-runtime      — pi-base + pi source + bun install      (DEFAULT, runnable)
#
# Build:
#     docker build -t nexus-agent/nexus:dev .                          # default = pi-runtime
#     docker build --target pi-base -t nexus-agent/nexus-base:dev .    # base for derived images
#
# Run:
#     docker run --rm nexus-agent/nexus:dev --help
#     docker run --rm -it -v "$PWD":/work nexus-agent/nexus:dev cli    # interactive nexus
#
...
############################
# 3) pi-base — python + bun + rustup + natives + omp_rpc + nexus shim
#
...
# `nexus` shim — runs the coding-agent CLI against $PI_ROOT via Bun. Derived
...
RUN printf '%s\n' \
    ...
    '  echo "nexus: PI_ROOT=$PI_ROOT does not look like a nexus-agent checkout" >&2' \
    '  exit 127' \
    'fi' \
...
#
# A self-contained, runnable nexus image. `docker run nexus-agent/nexus:dev --help`
...
ENTRYPOINT ["/usr/bin/tini", "--", "/usr/local/bin/nexus"]
CMD ["--help"]
