 ...
 *
 * - Tauri mode: uses `@tauri-apps/plugin-dialog` `open({ directory: true })`.
 * - Web mode (Chrome/Edge 86+): uses the File System Access API
 *   `showDirectoryPicker`. The returned path is a virtual `/<name>` since the
 *   browser can only expose the folder name, not the full host path.
 * - Fallback: prompts the user to type a path manually.
 *
...
export async function pickDirectory(
  ...
  try {
    // Tauri mode: native directory selector
    if (isTauri()) {
    ...
    // Web mode: File System Access API (Chrome/Edge 86+)
    if (
      ...
      // Browser only exposes the folder name; use a virtual path prefix
      return `/${handle.name}`;
    }
    ...
    console.error('[pickDirectory] Failed to pick folder:', err);
    return null;
  }
}
