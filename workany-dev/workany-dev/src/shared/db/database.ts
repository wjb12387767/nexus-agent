...
export async function getAnalyticsHeatmap(): Promise<HeatmapDay[]> {
  const database = await getSQLiteDatabase();

  if (database) {
    try {
      const rows = await database.select<{ d: string; c: number }[]>(
...
export async function getAnalyticsStats(): Promise<AnalyticsStats> {
  ...
  if (database) {
    try {
      const taskRow = await database.select<{ total: number; cost: number | null; duration: number | null }[]>(
        `SELECT COUNT(*) AS total, COALESCE(SUM(cost), 0) AS cost, COALESCE(SUM(duration), 0) AS duration FROM tasks`
      );
      const msgRow = await database.select<{ total: number }[]>(
        `SELECT COUNT(*) AS total FROM messages`
      );
      const fileRow = await database.select<{ total: number }[]>(
        `SELECT COUNT(*) AS total FROM files`
      );
      return {
        totalTasks: taskRow[0]?.total || 0,
        totalMessages: msgRow[0]?.total || 0,
        totalFiles: fileRow[0]?.total || 0,
        totalCost: taskRow[0]?.cost || 0,
        totalDuration: taskRow[0]?.duration || 0,
      };
    } catch {
      return {
        totalTasks: 0,
    ...
    const tx = db.transaction('messages', 'readonly');
    const store = tx.objectStore('messages');
    const allMsgs = await idbRequest(store.getAll());
    return {
      totalTasks: tasks.length,
      totalMessages: allMsgs.length,
      totalFiles: files.length,
      totalCost: tasks.reduce((s, t) => s + (t.cost || 0), 0),
      totalDuration: tasks.reduce((s, t) => s + (t.duration || 0), 0),
    };
  }
}

export async function getAnalyticsToolUsage(): Promise<ToolUsageItem[]> {
  const database = await getSQLiteDatabase();

  if (database) {
    try {
      const rows = await database.select<{ tool: string; c: number }[]>(
        `SELECT tool_name AS tool, COUNT(*) AS c
         FROM messages
         WHERE type = 'tool_use' AND tool_name IS NOT NULL
         GROUP BY tool_name
         ORDER BY c DESC
         LIMIT 20`
      );
    ...
    const tx = db.transaction('messages', 'readonly');
    const store = tx.objectStore('messages');
    const allMsgs = (await idbRequest(store.getAll())) as Message[];
    const map = new Map<string, number>();
    for (const m of allMsgs) {
      if (m.type === 'tool_use' && m.tool_name) {
        map.set(m.tool_name, (map.get(m.tool_name) || 0) + 1);
      }
    }
    return [...map.entries()]
...
export async function getAnalyticsTrend(): Promise<TrendDay[]> {
  const database = await getSQLiteDatabase();
...
// ============ History (Rewind / Edit) Operations ============

// Delete all messages after the given message id (exclusive) for a task
export async function deleteMessagesAfter(
  ...
  if (database) {
    const result = await database.execute(
      'DELETE FROM messages WHERE task_id = $1 AND id > $2',
      [taskId, messageId]
    );
    return result.rowsAffected;
  } else {
    ...
    const store = tx.objectStore('messages');
    const index = store.index('task_id');
    const messages = (await idbRequest(index.getAll(taskId))) as Message[];
    let deleted = 0;
    for (const m of messages) {
      if (m.id > messageId) {
        await idbRequest(store.delete(m.id));
        deleted++;
      }
    }
    return deleted;
...
export async function updateMessageContent(
  ...
  if (database) {
    ...
    const tx = db.transaction('messages', 'readwrite');
    const store = tx.objectStore('messages');
    const msg = (await idbRequest(store.get(messageId))) as Message | undefined;
    if (msg) {
...
// Get messages after the given message id (exclusive) for a task
export async function getMessagesAfter(
  ...
  if (database) {
    try {
      return database.select<Message[]>(
        'SELECT * FROM messages WHERE task_id = $1 AND id > $2 ORDER BY id ASC',
        [taskId, messageId]
      );
    } catch {
      return [];
