...
export interface NexusAgentSettings {
  // Core
  model: string;
  maxTurns: number;
  permissionMode: 'bypass' | 'ask' | 'always-allow';
  thinkingLevel: 'adaptive' | 'low' | 'medium' | 'high' | 'disabled';
  // Sandbox
  sandboxEnabled: boolean;
  sandboxFallbackBehavior: 'error' | 'warn' | 'continue';
  fileSafetyEnabled: boolean;
  doomLoopDetection: boolean;
  bashAstSafety: boolean;
  // Checkpoint
  ...
  mnemopiEnabled: boolean;
  // Self-Improvement
  learningGraphEnabled: boolean;
  autoLearnCapture: boolean;
  reflectOnErrors: boolean;
  // WSL Bridge
  wslAutoDetect: boolean;
  wslPreferredDistro: string;
  wslSuppressHint: boolean;
  // Advanced
  promptCaching: boolean;
  contextBreakdown: boolean;
  thinkScrubber: boolean;
  telemetry: boolean;
  debugMode: boolean;
}

export const defaultNexusSettings: NexusAgentSettings = {
  // Core
  model: 'claude-sonnet-4-20250514',
  maxTurns: 200,
  ...
  // Sandbox
  sandboxEnabled: true,
  sandboxFallbackBehavior: 'warn',
  fileSafetyEnabled: true,
  ...
  mnemopiEnabled: false,
  // Self-Improvement
  learningGraphEnabled: true,
  autoLearnCapture: true,
  reflectOnErrors: true,
  // WSL Bridge
  wslAutoDetect: true,
  wslPreferredDistro: '',
  wslSuppressHint: false,
  // Advanced
  promptCaching: true,
...
/**
 * Merge stored nexus settings with defaults to ensure all fields exist.
 * Handles migration from older settings that may be missing new fields.
 */
export function mergeNexusSettings(
  stored: Partial<NexusAgentSettings> | undefined
): NexusAgentSettings {
  if (!stored || typeof stored !== 'object') {
    return { ...defaultNexusSettings };
  }
  return { ...defaultNexusSettings, ...stored };
}
...
export interface Settings {
  ...
  // Nexus Agent settings
  nexusSettings?: NexusAgentSettings;
...
export const defaultSettings: Settings = {
  ...
  nexusSettings: defaultNexusSettings, // Nexus Agent settings
  theme: 'system',
  accentColor: 'orange',
...
export async function getSettingsAsync(): Promise<Settings> {
  ...
  if (database) {
    try {
      ...
      if (result.length > 0) {
        ...
        // Migration: Ensure nexusSettings has all fields
        settings.nexusSettings = mergeNexusSettings(settings.nexusSettings);
  ...
  try {
    ...
    if (stored) {
      ...
      // Migration: Ensure nexusSettings has all fields
      loadedSettings.nexusSettings = mergeNexusSettings(
        loadedSettings.nexusSettings
      );
      // Debug: Log loaded settings
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/providers/settings/sync`, {
      ...
      body: JSON.stringify({
        ...
        defaultModel: settings.defaultModel,
        // Nexus Agent settings
        nexusSettings: settings.nexusSettings,
      }),
    });
