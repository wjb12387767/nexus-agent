...
function TaskDetailContent() {
  ...
  // Workspace directory (from user settings) — shown in header and toolbar
  const [workspaceDir, setWorkspaceDir] = useState<string>(
    () => getSettings().workDir || ''
  );
  const handleWorkspaceChange = useCallback((dir: string) => {
    const current = getSettings();
    if (current.workDir === dir) return;
    const next = { ...current, workDir: dir };
    void saveSettingsWithSync(next);
    setWorkspaceDir(dir);
    console.log('[TaskDetail] Workspace changed to:', dir);
  }, []);
        ...
        >
          ...
          >
            ...
            <header className="border-border/50 bg-background z-10 flex shrink-0 items-center gap-2 border-none px-4 py-3">
              ...
              {workspaceDir && (
                <span
                  className="text-muted-foreground hidden shrink-0 items-center gap-1.5 rounded-md px-2 py-1 text-xs sm:inline-flex"
                  title={workspaceDir}
                >
...
function UserMessage({
  ...
  attachments?: MessageAttachment[];
  dbId?: number;
  onRewind?: (messageId: number) => Promise<void>;
  ...
  const [isRewindConfirm, setIsRewindConfirm] = useState(false);
  ...
  const canAct = !!dbId && !isRunning && !isProcessing;
  const showActions = canAct && (onRewind || onEdit);

  const handleStartEdit = () => {
    setEditValue(content);
    setIsEditing(true);
    setIsRewindConfirm(false);
  };
  ...
  const handleSaveEdit = async () => {
    if (!dbId || !onEdit) return;
    const trimmed = editValue.trim();
    if (!trimmed) return;
    setIsProcessing(true);
    try {
      await onEdit(dbId, trimmed);
    } finally {
  ...
  return (
    <div className="group flex min-w-0 gap-3">
      ...
      <div className="relative max-w-[85%] min-w-0">
        {/* Hover action buttons - absolutely positioned to avoid layout shift */}
        {showActions && !isEditing && !isRewindConfirm && (
          <div className="absolute right-full top-0 flex items-center gap-0.5 pr-1 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
            {onEdit && (
              <button
                type="button"
                onClick={handleStartEdit}
                title={t.settings.historyEditTitle}
            ...
            {onRewind && (
              <button
                type="button"
                onClick={() => setIsRewindConfirm(true)}
                title={t.settings.historyRewindTitle}
                className="text-muted-foreground hover:bg-muted hover:text-foreground rounded-md p-1.5 transition-colors"
              >
        ...
        {/* Rewind confirmation popover */}
        {showActions && isRewindConfirm && (
          <div className="bg-card border-border absolute right-full top-0 flex w-56 flex-col gap-1.5 rounded-lg border p-2 shadow-sm">
            <span className="text-muted-foreground text-xs">
              {t.settings.historyRewindConfirm.replace(
                '{count}',
                String(subsequentCount ?? 0)
              )}
            </span>
            <div className="flex items-center justify-end gap-1">
              <button
                ...
                className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-md px-2 py-1 text-xs font-medium transition-colors disabled:opacity-50"
              >
                {t.settings.historyConfirm}
              </button>
              <button
                type="button"
                onClick={() => setIsRewindConfirm(false)}
                disabled={isProcessing}
                className="text-muted-foreground hover:bg-muted hover:text-foreground rounded-md px-2 py-1 text-xs transition-colors disabled:opacity-50"
              >
                {t.settings.historyCancel}
              </button>
            </div>
          </div>
        )}
          ...
          {isEditing ? (
            <div className="flex flex-col gap-2">
              <textarea
                ...
                className="bg-background text-foreground focus:ring-primary min-w-0 resize-y rounded-lg border p-2 text-sm outline-none focus:ring-1"
                onKeyDown={(e) => {
      ...
      {groups.map((group, index) => {
        ...
        return (
          <MessageItem
            ...
            isRunning={isRunning}
            subsequentCount={
              messages.indexOf(group.message) >= 0
                ? messages.length - messages.indexOf(group.message) - 1
                : 0
            }
          />
        );
