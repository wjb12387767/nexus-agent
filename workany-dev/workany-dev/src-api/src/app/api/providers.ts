...
interface SettingsSyncBody {
  ...
  defaultModel?: string;
  nexusSettings?: Record<string, unknown>;
}
...
providersRoutes.post('/settings/sync', async (c) => {
  ...
  // Merge nexusSettings into agentConfig so the NexusAgent adapter
  // receives them via its config when the agent provider is switched.
  const mergedAgentConfig: Record<string, unknown> = {
    ...(body.agentConfig || {}),
  };
  if (body.nexusSettings) {
    mergedAgentConfig.nexusSettings = body.nexusSettings;
  }

  if (body.agentProvider) {
    await manager.switchAgentProvider(body.agentProvider, mergedAgentConfig);
  }
  ...
  console.log('[ProvidersAPI] Settings synced:', {
    ...
    hasNexusSettings: !!body.nexusSettings,
  });
