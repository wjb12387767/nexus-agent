...
预构建的多架构镜像(`linux/amd64` + `linux/arm64`)在每次 `nexus-v*` tag push 时发布到 GitHub Container Registry。
...
# 3. (可选)启动完整栈(agent + Qdrant + Docling + LightRAG)
docker compose up -d
docker compose exec nexus cli
```

**完整栈模式**通过 [docker-compose.yml](./docker-compose.yml) 编排全部 4 个 MCP 服务:

1. 把 [mcp.json.example](./mcp.json.example) 复制到 `~/.nexus/agent/mcp.json`
2. 填入你的 API key(`LLM_API_KEY`、`LLM_API_BASE`、`LLM_MODEL`)
3. `docker compose up -d`

compose 文件把 `${WORKSPACE_DIR:-./workspace}` 挂载到 `/work`,并把 `~/.nexus`(凭证、会话、mnemopi DB)持久化跨重建。
