...
rm -rf /srv/agent-home/.agent /srv/agent-home/.nexus/agent
mkdir -p /srv/agent-home/.agent /srv/agent-home/.nexus/agent
if [ -e /srv/agent-home-stage/.agent ]; then
    cp -a /srv/agent-home-stage/.agent/. /srv/agent-home/.agent/
fi
if [ -e /srv/agent-home-stage/.nexus/agent ]; then
    cp -a /srv/agent-home-stage/.nexus/agent/. /srv/agent-home/.nexus/agent/
fi
...
# nexus registers daemon project presence under ~/.nexus/run at startup, nesting
...
mkdir -p /srv/agent-home/.nexus/run
chgrp -R omp /srv/agent-home/.nexus/run
chmod -R g+rwX /srv/agent-home/.nexus/run
find /srv/agent-home/.nexus/run -type d -exec chmod g+s {} +
chmod 2770 /srv/agent-home/.nexus/run
