...
bundles. Bun's `fetch` does not consume the env var natively, so nexus injects
...
Authenticate via the [`nexus`](https://nexus.agent) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
nexus auth-broker login              # interactive provider selection
nexus auth-broker login anthropic    # login to a specific provider
nexus auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
nexus auth-broker list               # list supported providers
nexus auth-broker logout             # interactive — pick a stored credential to remove
```
