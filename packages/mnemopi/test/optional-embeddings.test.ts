	...
	it("calls an OpenAI-compatible custom embeddings endpoint without requiring an API key", async () => {
		...
		const server = Bun.serve({
			...
			fetch: async request => {
				...
				expect(request.headers.get("user-agent")).toBe(`Nexus-Agent/${packageJson.version}`);
				expect(request.headers.get("http-referer")).toBe("https://nexus.agent/");
				expect(request.headers.get("x-openrouter-title")).toBe("Nexus-Agent");
