...
export class TaskTool implements AgentTool<TaskToolSchemaInstance, TaskToolDetails, Theme> {
	...
	async #runSpawn(
		...
		// M7: per-agent 模型路由（Nexus Routing）。
		// 优先级：explicit（task 工具显式指定 model，且为 agentModels key）
		//   > routing（agentRouting 按 name/subagentType 命中）
		//   > default（agentRouting.default 命中）
		//   > global（未命中，回落到下方原有的 omp 模型解析路径）
		// 命中时把 agentModels 条目注册为 omp ModelRegistry 的合成 provider
		// （`nexus-routing`），并把 `modelPattern` 作为 `modelOverride` 喂给
		// omp 标准解析路径，让后续 `resolveModelOverride` / subagent
		// subprocess 能通过 `nexus-routing/<routeKey>` selector 命中。
		const nexusRoutingResolution = resolveNexusRoutingForSpawn({
			modelRegistry: this.session.modelRegistry,
			toolSpecifiedModel: undefined, // task 工具自身不暴露 model 参数；保留接口
			agentName,
			subagentType: agentName,
			agentDefinitionModel: effectiveAgent.model,
		});

		const modelOverride = nexusRoutingResolution
			? [nexusRoutingResolution.modelPattern]
