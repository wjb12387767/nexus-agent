...
export const SETTINGS_SCHEMA = {
	...
	"bash.astSecurity": {
		...
		ui: {
			tab: "shell",
			group: "Bash",
			label: "Bash AST Security",
			description:
				"Use @nexus-agent/bash-ast AST analysis as the primary approval signal; falls back to CRITICAL_BASH_PATTERNS regex on parser failure or exception",
		},
	},
	...
	// ────────────────────────────────────────────────────────────────────────
	// Sandbox (OS-level isolation for bash tool subprocesses)
	// ────────────────────────────────────────────────────────────────────────
	"sandbox.enabled": {
		type: "boolean",
		default: false,
		ui: {
			tab: "shell",
			group: "Sandbox",
			label: "Enable OS Sandbox",
			description:
				"Apply kernel-enforced filesystem sandbox to bash tool subprocesses (Landlock on Linux, Seatbelt on macOS, ISO FS fallback on Windows)",
		},
	},

	"sandbox.profile": {
		type: "enum",
		values: ["workspace", "devbox", "read-only", "strict", "off", "custom"] as const,
		default: "workspace",
		ui: {
			tab: "shell",
			group: "Sandbox",
			...
			description:
				"Built-in profile (workspace/devbox/read-only/strict), off (no sandbox), or custom (defined in ~/.nexus/sandbox.toml)",
			options: [
				{
					value: "workspace",
					label: "Workspace",
					description: "Read-all + write only to workspace, ~/.nexus, /tmp",
				},
				{
					value: "devbox",
					label: "Devbox",
					description: "Read-all + write to most top-level dirs (except /data)",
				},
				{
					value: "read-only",
					label: "Read-only",
					description: "Read-all + minimal writes (only ~/.nexus + /tmp), network blocked",
				},
				{
					value: "strict",
					label: "Strict",
					description: "No default read, only explicit system paths, network blocked",
				},
				{ value: "off", label: "Off", description: "Disable sandbox even when sandbox.enabled is true" },
				{
					value: "custom",
					label: "Custom",
					description: "Use a custom profile defined in ~/.nexus/sandbox.toml or .nexus/sandbox.toml",
				},
			],
		},
	},
	...
	// Nexus 文件级 checkpoint/rewind 系统（M3 里程碑）
	// 与上面的会话级 checkpoint.enabled 互补：本系统真正还原磁盘文件内容
	"checkpoint.autoEnabled": {
		type: "boolean",
		default: false,
		ui: {
			tab: "tools",
			group: "File Checkpoint",
			label: "Auto File Checkpoint",
			description:
				"Automatically create a file-level checkpoint before bash/edit/write tools modify files. Enables /rewind to restore disk state.",
		},
	},
	"checkpoint.autoInterval": {
		...
		ui: {
			tab: "tools",
			group: "File Checkpoint",
			label: "Auto Checkpoint Interval (seconds)",
			description:
				"Minimum seconds between auto checkpoints. 0 = always checkpoint before every tool call. Prevents excessive checkpoints during rapid edits.",
		},
	},
	"checkpoint.maxSizeMb": {
		...
		ui: {
			tab: "tools",
			group: "File Checkpoint",
			label: "Max Checkpoint Size (MB)",
			description:
				"Maximum total disk usage for checkpoint blobs. When exceeded, oldest checkpoints are evicted by the swap policy.",
		},
	},
	"checkpoint.swapPolicy": {
		...
		ui: {
			...
			label: "Swap Policy",
			description:
				"Checkpoint eviction policy: 'lru' (least recently used), 'lru-size' (LRU + size cap), 'fifo' (oldest first), 'none' (no auto eviction).",
		},
	},
	...
	"dev.autoqaPush.endpoint": {
		type: "string",
		default: "https://qa.nexus.agent/v1/grievances" as const,
		ui: {
			tab: "tools",
			group: "Developer",
			label: "Auto QA Push Endpoint",
			description: "Full URL receiving Auto QA JSON reports (default https://qa.nexus.agent/v1/grievances)",
		},
	},
