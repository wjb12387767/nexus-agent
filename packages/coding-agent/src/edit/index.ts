...
import { maybeAutoCheckpoint } from "../tools/file-checkpoint";
...
export class EditTool implements AgentTool<TInput> {
	...
	async execute(
		...
		// 在 edit 执行前自动打 checkpoint，仅在 `checkpoint.autoEnabled = true`
		...
		if (targetPath && targetPath !== "(unknown)") {
			await maybeAutoCheckpoint(this.session, [targetPath]).catch(() => {
				// checkpoint 失败不应阻塞 edit 执行
			});
		}
