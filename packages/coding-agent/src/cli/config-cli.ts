...
import { runRoutingCommand } from "./routing-cli";

// =============================================================================
// Types
// =============================================================================

export type ConfigAction = "list" | "get" | "set" | "reset" | "path" | "init-xdg" | "routing";
...
export interface ConfigCommandArgs {
	...
	value?: string;
	/**
	 * 仅 `action === "routing"` 时使用：包含 `nexus config routing` 之后的
	 * 全部位置参数与子命令（保留原始数组形态，不做 join，因为 base_url /
	 * api_key 可能含特殊字符）。
	 */
	rest?: string[];
	flags: {
...
const VALID_ACTIONS: ConfigAction[] = ["list", "get", "set", "reset", "path", "init-xdg", "routing"];
...
export function parseConfigArgs(args: string[]): ConfigCommandArgs | undefined {
	...
	// routing action 透传全部剩余 token（含 flags）给 routing-cli.ts 自解析，
	// 不走 settings 的 key/value 解析路径。
	if (action === "routing") {
		result.rest = args.slice(2);
		return result;
	}
...
export async function runConfigCommand(cmd: ConfigCommandArgs): Promise<void> {
	// routing action 不依赖 omp settings，跳过 Settings.init() 直接走
	// routing-cli.ts 的独立解析路径（操作 ~/.nexus/config.json）。
	if (cmd.action === "routing") {
		await runRoutingCommand(cmd.rest ?? []);
		return;
	}
...
${chalk.bold("Commands:")}
  ...
  routing <sub>      Manage per-agent model routing (run \`${APP_NAME} config routing help\`)
