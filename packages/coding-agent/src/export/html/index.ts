...
export interface ExportOptions {
	 ...
	 * - `"web"` (default) — the nexus brand identity (collab-web pink/purple),
	 *   so public HTML exports and the `/s/<id>` share viewer match the live
	 *   `collab.nexus.agent` client. See `web-palette.ts`.
 ...
 *     `"web"` (the default for public artifacts) returns the fixed nexus brand
 *     palette from `web-palette.ts` — collab-web pink/purple identity, shared
 *     with the live `collab.nexus.agent` client, so exports and the share viewer render
