...
Core implementation package for the `nexus` coding agent in the `oh-my-pi` monorepo.
...
The agent supports three mutually-exclusive memory backends, selected via the `memory.backend` setting (Settings → Memory tab, or `~/.nexus/config.yml`):
