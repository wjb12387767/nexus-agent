/**
 * `nexus join <link>` must route to the registered `join` subcommand instead of
...
describe("join command is registered as a top-level subcommand", () => {
	test("CLI runner routes `join <link>` to the join command, not launch", () => {
		...
		expect(resolveCliArgv(["join", "wss://collab.nexus.agent/s/abc#key"])).toEqual({
			argv: ["join", "wss://collab.nexus.agent/s/abc#key"],
		});
	});
});
