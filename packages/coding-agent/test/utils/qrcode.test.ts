...
describe("QR encoder", () => {
	...
	const vectors: ReadonlyArray<{
		...
		{
			text: "https://collab.nexus.agent/#mgAYTZwEnpRQtca0CTgn-Q.gdJU",
			ecl: "M",
			mask: 4,
			version: 4,
			size: 33,
			hash: "2657fb33632c6d1d",
		},
		{
	...
	it("deterministically selects a penalty-minimizing mask when none is forced", () => {
		...
		const qr = QrCode.encodeText("https://collab.nexus.agent/#demo", "M");
		expect(qr.mask).toBe(3);
		expect(matrixFingerprint(qr)).toBe("aa527fc4e4cf9aee");
	});
...
describe("renderQrHalfBlocks", () => {
	it("frames the symbol in a light quiet zone wide enough for the margin", () => {
		const qr = QrCode.encodeText("https://nexus.agent/#demo", "M");
		const margin = 3;
