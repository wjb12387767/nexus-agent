...
describe("normalizeShareServerUrl", () => {
	test("strips trailing slashes and falls back to the default", () => {
		expect(normalizeShareServerUrl("https://collab.nexus.agent/s/")).toBe("https://collab.nexus.agent/s");
		expect(normalizeShareServerUrl("https://example.com/s///")).toBe("https://example.com/s");
		expect(normalizeShareServerUrl(undefined)).toBe("https://collab.nexus.agent/s");
		expect(normalizeShareServerUrl("   ")).toBe("https://collab.nexus.agent/s");
	});
});
