...
async function runLoopBody(
	...
	try {
		...
		// Doom Loop 检测器：跟踪跨 turn 的工具调用滑动窗口，连续相同签名时
		// 通过 logger.warn 提示模型改变策略。与 soft-escalation/noop-guard 正交，
		// 仅作提示不阻断执行。实例化一次贯穿整个 runLoop，跨 turn 共享窗口。
		const doomLoopDetector = new DoomLoopDetector(config.doomLoopDetection);
