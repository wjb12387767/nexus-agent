...
import { precompactWithNexus, type NexusPrecompactMode } from "./nexus-precompact";
...
export interface CompactionSettings {
	...
	/**
	 * Nexus 局部预压缩模式（在 omp LLM 摘要前先做结构化压缩）。
	 *
	 * - `off`（默认）：不启用，完全走 omp 原生流程（向后兼容）
	 * - `code`：仅代码块压缩（保留签名 + 占位符）
	 * - `code-intra`：代码块 + 单 turn 内重复合并
	 * - `code-intra-inter`：完整局部压缩（代码块 + 单 turn + 跨 turn）
	 *
	 * 仅做无 LLM 的结构化压缩，全局语义摘要仍由 omp 原生 strategy 负责。
	 */
	nexusPrecompact?: NexusPrecompactMode;
}
...
export async function compact(
	...
	// 单独提取为 let，以便 nexus 预压缩可替换
	let messagesToSummarize = preparation.messagesToSummarize;

	const reserveTokens = settings.reserveTokens ?? DEFAULT_RESERVE_TOKENS;

	// Nexus 局部预压缩（可选，默认 off）：在 omp LLM 摘要前先做无 LLM 的结构化压缩
	const nexusMode = settings.nexusPrecompact ?? "off";
	if (nexusMode !== "off" && messagesToSummarize.length > 0) {
		const precompactResult = precompactWithNexus(messagesToSummarize, nexusMode);
		if (precompactResult.applied) {
			messagesToSummarize = precompactResult.messages;
			logger.debug("nexus precompaction applied", precompactResult.stats);
		}
	}
