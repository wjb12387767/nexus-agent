...
import type { DoomLoopConfig } from "./doom-loop-detector";
...
export interface AgentLoopConfig extends SimpleStreamOptions {
	 ...
	 * Doom Loop 检测配置（可选）。
	 *
	 * 跟踪最近 N 个工具调用的 (toolName, argsSignature) 滑动窗口，当末尾出现
	 * 连续 K 个完全相同签名的调用时记录一条 warn 级别日志，提示模型改变策略。
	 * 与 MAX_SOFT_TOOL_ESCALATIONS、noop-loop-guard 正交，互不调用。
	 *
	 * 触发后 detector 自动 reset，避免对同一循环重复告警。仅作提示，不阻断
	 * 工具执行。默认启用（参考 DEFAULT_DOOM_LOOP_CONFIG）。
	 */
	doomLoopDetection?: Partial<DoomLoopConfig>;
}
