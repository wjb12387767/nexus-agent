...
1. **Local nexus, not npm.** By default the runner bind-mounts the repo
   read-only into each task container (`--install source`) and runs nexus
   ...
   `/opt/nexus/bin` — so trial setup needs zero outbound network. Alternatives:
...
| `--agent-arg <arg>` | — | Extra arg forwarded verbatim to the in-container nexus CLI (repeatable) |
| `--env <KEY[=VALUE]>` | — | Forward env into the nexus container (repeatable); `KEY` alone forwards the host value |
| `--binary <path>` | — | Prebuilt nexus binary (repeat for arm64+x64) |
...
arc; ~$0.001 per report). API keys resolve through nexus's auth storage.
