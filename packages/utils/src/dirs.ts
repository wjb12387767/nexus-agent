/**
 * Centralized path helpers for nexus config directories.
 *
 * Uses PI_CONFIG_DIR (default ".nexus") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/nexus/. This requires running `nexus config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, nexus trusts that the migration has been done.
 *
 * M8 breaking change: config root moved from `~/.nexus/` to `~/.nexus/`.
 * Users upgrading from omp must manually `mv ~/.omp ~/.nexus`.
 */
...
/** App name (e.g. "nexus") */
export const APP_NAME: string = "nexus";

/** Config directory name (e.g. ".nexus") */
export const CONFIG_DIR_NAME: string = ".nexus";
...
/** Get the config directory name relative to home (e.g. ".nexus" or PI_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.PI_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".nexus/agent" or PI_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all nexus directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/nexus/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/nexus/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/nexus once `nexus config init-xdg` has migrated
		...
		// profile could resolve to `~/.nexus/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/nexus exists yet) and then silently move to
		// `$XDG_*_HOME/nexus/profiles/<name>` the moment the base appeared, orphaning
		...
		// XDG flattens the agent/ prefix: ~/.nexus/agent/sessions → $XDG_DATA_HOME/nexus/sessions
		this.#agentDirs = {
 ...
 * `~/.nexus/agent`. Captured at module load — ignoring a profile-derived value
...
/** Get the config root directory (~/.nexus). */
...
/** Get the agent config directory (~/.nexus/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.nexus). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.nexus/*)
// =============================================================================

/** Get the reports directory (~/.nexus/reports). */
...
/** Get this process's dated log path (~/.nexus/logs/nexus.YYYY-MM-DD.PID.log). */
...
/**
 * Get the plugins directory (~/.nexus/plugins or its XDG equivalent).
 *
 ...
 * isolation, and `nexus worktree` cleanup all read the same base). Driven by the
 * `worktree.base` setting in coding-agent; pass `undefined`/empty to clear and
 * fall back to `OMP_WORKTREE_DIR` or the `~/.nexus/wt` default.
 *
 ...
 * `worktree.base` setting), then the `~/.nexus/wt` default. The env var and the
...
/** Get the SSH control socket directory (~/.nexus/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.nexus/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.nexus/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.nexus/agent/python-gateway; XDG default: $XDG_STATE_HOME/nexus/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.nexus/puppeteer). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.nexus/wt/`).
...
/** Get the path to a single worktree directory (~/.nexus/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.nexus/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.nexus/cache/github-cache.db).
...
/**
 * Get the encrypted auth-broker snapshot cache path (~/.nexus/cache/auth-broker-snapshot.enc).
...
// =============================================================================
// Agent subdirectories (~/.nexus/agent/*)
// =============================================================================
...
/** Get the last-seen-changelog-version marker file (~/.nexus/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.nexus/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.nexus/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/nexus/cache/document-conversions). */
...
/** Get the project-level prompts directory (.nexus/prompts). */
export function getProjectPromptsDir(cwd: string = getProjectDir()): string {
	return path.join(getProjectAgentDir(cwd), "prompts");
}

/** Get the project-level plugin overrides path (.nexus/plugin-overrides.json). */
...
/**
 * Persistent per-install UUID stored at `~/.nexus/install-id`.
 *
 ...
 * `~/.nexus/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.nexus/install-id`) regardless of the
