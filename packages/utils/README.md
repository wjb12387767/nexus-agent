...
| Module | Purpose |
| --- | --- |
| `logger` | Centralized logger writing to `~/.nexus/logs/` with rotation (TUI-safe — never stdout) |
| `prompt` | Handlebars-based prompt templating and formatting helpers |
| `dirs` | Path helpers for nexus config directories (`~/.nexus`, XDG-aware on Linux) |
